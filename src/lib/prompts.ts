...
- 배운 점/인사이트: ${detail?.learning}
    ...
    const lengthInstruction = targetQuestion.maxChars
        ? `글자 수 제한: 공백 포함 ${targetQuestion.maxChars}자. **반드시 ${Math.floor(targetQuestion.maxChars * 0.9)}자 ~ ${targetQuestion.maxChars}자 사이**로 작성하십시오. 분량을 채우기 위해 의미 없는 수식어를 늘리지 말고, 경험의 디테일과 당신의 생각, 인사이트를 풍부하게 서술하십시오.`
        : "";
...
4. **90% 분량 준수**: 설정된 글자 수의 90% 미만으로 작성할 시 실패로 간주합니다. 경험의 디테일을 더 깊게 파고들어 충분한 분량과 깊이감을 확보하십시오.
