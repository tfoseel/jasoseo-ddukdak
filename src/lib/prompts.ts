...
[작성 원칙]
1. AI 클리셰 제거: "열정", "최선", "노력", "성장", "시너지"와 같은 상투적이고 추상적인 단어는 절대 사용하지 마십시오.
2. 구체적 고유명사 활용: 인터뷰에서 제공된 회사명, 팀 이름, 프로젝트 명칭, 구체적 기술 스택, 숫자(%, 개월, 명)를 적극적으로 문장에 녹여내십시오.
3. 역전된 구조: 결과(성과)를 문장의 앞부분에 배치하여 임팩트를 주십시오.
4. 짧은 호흡: 한 문장은 최대 2~3줄을 넘지 않도록 짧고 명료하게 작성하십시오. 
5. 솔직함 수용: 사용자가 답변한 '솔직한 회고(실패나 아쉬운 점)'가 있다면 이를 숨기지 말고, 어떻게 극복했는지 혹은 무엇을 배웠는지로 연결하여 진정성을 확보하십시오.

[톤앤매너]
사용자가 선택한 톤에 따라 문체를 엄격히 구별하십시오:
- 논리적/분석적: 데이터와 근거 중심, '~함', '~임' 등의 담백한 종결어미 사용 가능. 전문 용어의 정확한 사용.
- 열정적/주도적: 행동 중심의 동사 사용, 에너지 있는 문체, 도전적인 목표와 성취 강조.
- 부드러운/협력적: 관계와 소통 중심, 유연한 표현, 조직 융화를 강조하는 문체.

[금지 사항]
- "~하는 과정을 통해", "~를 배울 수 있었습니다"와 같은 늘어지는 설명형 문조 지양.
- "입사 후 포부" 같은 뻔한 제목 대신, 내용의 핵심을 관통하는 소제목을 활용하십시오.
`;

export function generateUserPrompt(data: InterviewData): string {
...
[작성할 문항 리스트]
${basicInfo.questions.map((q, i) => `${i + 1}. ${q}`).join("\n")}
...
위 정보를 바탕으로 각 문항에 대한 초안을 작성해주세요. 
각 문항별로 구분하여 출력하되, 문항 사이에는 반드시 "---DRAFT_SEPARATOR---" 라는 구분자를 넣어주세요.
가독성을 위해 각 문항에는 적절한 소제목(예: [소제목])을 붙여주세요.
`;
}
